...
CASES = ('normal', 'timeout', 'operator_stop', 'controller_crash',
         ...
         'slow_stream', 'active_request_stop', 'partial_start', 'cleanup_retry', 'cli_run', 'cli_cleanup_retry', 'normal_repeat')
...
from pathlib import Path
# The mounted workspace root cannot receive copytree's final chmod/copystat.
for item in Path('/fixtures/reference').iterdir():
    target = Path('/workspace') / item.name
...
def controller(root, case):
    ...
    def docker(*args, **kwargs):
        args = list(args)
        if case in ('cleanup_retry','cli_cleanup_retry') and args[:2] == ['network','rm']:
        ...
        if args[0] == 'create' and '--name' in args:
            ...
            elif name.startswith('s2-worker-') and case in ('timeout','operator_stop','controller_crash','cli_run','cli_cleanup_retry'):
                at = args.index(lock['images']['worker'])
                args = args[:at] + [*runtime.mount(root/'fixtures','/fixtures',True),
                    lock['images']['worker'],'python3','/fixtures/cli-worker.py' if case.startswith('cli_') else '/fixtures/worker.py']
    ...
    with patch.object(profiles, 'resolve', resolve), patch.object(runtime, 'docker', docker), \
        ...
        if case.startswith('cli_'):
            ...
            if code != (1 if case == 'cli_cleanup_retry' else 0):
                raise RuntimeError('Ordinary run CLI returned an unexpected exit code')
            return
...
def main():
    ...
    util.write_new_json(batch/'plan.json', {'synthetic':True, 'model_called':False,
        'cases':list(args.cases), 'controller_files':runtime.controller_files(REPO),
    ...
    for case in args.cases:
        ...
        if case.startswith('cli_'): util.collect(REPO/'inner/fixtures/reference', fixtures/'reference')
        ...
        if case in ('cleanup_retry','cli_cleanup_retry'):
            ...
            stopped_cli = subprocess.run([sys.executable,'-m','outer.harness.cli','--runs-dir',str(root),
                'stop','--run',rid], cwd=REPO, env=child_environment(), capture_output=True, timeout=65)
            (root/'retry-stop-cli.stdout').write_bytes(stopped_cli.stdout)
            (root/'retry-stop-cli.stderr').write_bytes(stopped_cli.stderr)
            if stopped_cli.returncode: raise RuntimeError('Cleanup retry stop CLI failed')
            manifest = run.load_manifest(root, rid)
            if not manifest['network_cleanup']['confirmed'] or any(manifest[k] != v for k,v in prior_interval.items()):
        ...
        if case in ('normal','normal_repeat','slow_stream','cleanup_retry','cli_run','cli_cleanup_retry'): passed = passed and usage['usage_complete'] and usage['input_reached']
        ...
        result.update(network_cleanup=manifest.get('network_cleanup'), network_inventory_unchanged=after == networks_before,
                      first_cleanup=first_cleanup if case in ('cleanup_retry','cli_cleanup_retry') else None)
        if case.startswith('cli_'):
            reference = util.read_json(run_root/'archive-reference.json')
            from harness import evaluate, preserve
            archived = preserve.verify(root/'_archive', reference['package_id'], reference['sha256'])
            result['ordinary_cli'] = {'run':util.read_json(root/'cli-result.json'),
                'scoring_state':evaluate.last_scoring(run_root)['scoring_state'],
                'archive_files':len(archived['files'])}
            passed = passed and result['ordinary_cli']['scoring_state'] == 'scored' and bool(archived['files'])
