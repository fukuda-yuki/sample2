...
def coverage_complete(output):
    return (output.get('browserCartCoverage') == OBSERVED
...
def stored_coverage_complete(directory, instance, artifact_hash, spec_hash):
    ...
    try:
        ...
        if (not coverage_complete(output) or output.get('reviewRunInstanceId') != instance
        ...
        if (receipt.get('runInstanceId') != instance or receipt.get('artifactSha256') != artifact_hash
                ...
                or sorted(r['checkId'] for r in receipt['removals']) != ['C-015', 'C-016']): return False
...
def compose_evaluation(condition, frozen, baseline, assets, out, instance, sequence):
    ...
    command[image_at:image_at] = runtime.mount(baseline, '/baseline', True)
    ...
    try:
        ...
        runtime.docker('rm', '-f', name, check=False)
    ...
    if output.get('browserCartEvidenceSha256') != util.sha256_file(out/'browser-cart/receipt.json'):
...
def complete_evaluation(repo, condition, frozen, baseline, published, assets, out,
    ...
    network = name + '-net'
    container_created = network_created = False
    image = condition['runtime_lock']['images']['evaluator']
    version = condition['evaluation']['evaluation_version']
    ...
    try:
        ...
        runtime.docker('network', 'create', '--opt', 'com.docker.network.bridge.enable_ip_masquerade=false',
                       '--label', 'sample2.browser-review=' + name, network)
        network_created = True
        command = ['docker', 'run', '-d', '--name', name, '--label', 'sample2.browser-review=' + name,
        ...
        return compose_evaluation(condition, frozen, baseline, assets, out, instance, sequence)
    except Exception as exc:
        util.write_new_json(out/'browser-fault.json', {'type': type(exc).__name__, 'message': str(exc), 'scoring_state': 'evaluator_fault'})
        # Never expose the HTTP-only pass as the result of a failed browser phase.
        if not (out/'evaluation.json').exists():
            fault = {**original, 'verdict': 'error', 'quality': None, 'researchStatus': 'incomplete',
                     ...
                     'reviewRunInstanceId': instance, 'evaluatorFaults': [str(exc)],
                     'observationScope': 'Browser phase incomplete; saved HTTP output remains in baseline'}
            util.write_new_json(out/'evaluation.json', fault)
        return 2
    finally:
        util.write_new_json(out/'browser-intent.json', intent)
        if container_created:
            logs = runtime.docker('logs', name, check=False)
            (out/'browser-server.log').write_text(logs.stdout + logs.stderr, encoding='utf-8')
            stopped = runtime.docker('rm', '-f', name, check=False)
            intent['container_removed'] = stopped.returncode == 0
        if network_created:
            removed = runtime.docker('network', 'rm', network, check=False)
            intent['network_removed'] = removed.returncode == 0
        util.write_new_json(out/'browser-cleanup.json', {k: v for k, v in intent.items() if k.endswith('_removed')})
