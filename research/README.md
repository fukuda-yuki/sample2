...
approval bound to the exact frozen plan. Earlier unallocated v2 records remain
historical; use the new execution-plan path explicitly for analysis.
...
identical offline extraction. Its pair-admission rule is now wired to the driver;
...
The underlying scientific design is the [catalog comparison v2 revision](../docs/ms1-catalog-comparison-v2-plan.md):
...
No model acquisition is authorized by preparation. The
...
- `catalog_execution`: freeze the 320-pair plan, check current pins/storage,
  ...
  only unsent slots, and hold between pairs until verified publication/cleanup.
- `catalog_delivery`: package a reviewed pair, reconcile Release assets, download
  independently, restore/extract offline and remove only verified owned scratch.
- `catalog_storage`: compress only stopped Run/package directories with NTFS,
  verify unchanged hashes, and retain all original logical bytes.
- `catalog_share`: stage one allocated pair, scan public copies, bind a review
...
- `catalog_admission`: check local storage, an outstanding API recovery pause,
  ...
  applied by the acquisition controller before each dispatch.
