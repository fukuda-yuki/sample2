...
| 別の Run の使用量を混ぜない | イベントは**その Run の `run_id` に束縛**する。`run_id` が一致しないイベントは正規化の入力にしない（記録には残し、`missing` に理由を積む） |

`usage/provenance.json` に、原本の出所、取得方法、`expected_sessions`、`inventory_complete` の根拠を記録する。
**`inventory_complete` の根拠を示せない限り `true` にしない。**

**`run_id` の一致を要求する理由。** 実行 A の 125 tokens を実行 B の完全な使用量として記録できてしまうと、
「その Run が使った量」ではなく「手元にあった量」が記録される。
`expected_sessions` だけでは足りない（セッション ID が使い回される場合がある）。
外側は Run の識別子（§3.3）を `normalize` に渡し、**一致しないイベントは数えない。**
...
**固定した成果物が変更されていないことも、採点の前に確かめる。** `frozen/` の回収時に計算した
`artifact_sha256_frozen` を `record.json` に残し、採点のたびに `frozen/` を計算し直して照合する。
一致しなければ**採点しない**（`scoring.state = rejected_mismatch`）。
「同じ `artifact_sha256` を申告した評価器の結果」だけを見ていると、
**固定した後に書き換えられた成果物**をそのまま採点してしまう。
...
**採点ごとに別の作業ディレクトリを与える。** 作業ディレクトリの位置は連番を含め、
`record.json` の `work_dir` に残す。同じ作業ディレクトリを使い回すと、
2 回目の採点が前回の DB を引き継ぎ、「毎回空の DB から始める」という初期条件（仕様 §4.1）を満たさない。
実際に、2 回の採点で注文が 3 件 → 6 件に増えることを確認している。
...
実評価器での確認**、**固定値による採点の拒否の実評価器での確認**、**評価器が `exit 2` を返したときの
外側の扱いの実評価器での確認**は、非モデルの自動検証では
確認していない（前者は [`outer/tests`](../outer/tests)
の単体テスト、後ろ 4 つは代替評価器のみ）。`V-12` が確かめるのは「この検証が残骸を残さなかった
...
8. **採点が成果物を書き換えないことの確認は、正例 1 件の成果物に限る**（`V-14`）。
   `bin` `obj` をプロジェクト直下に置く既定の SDK 構成での確認であり、
   出力先を変えた成果物や複数プロジェクトを含む成果物では確認していない
   （[`outer/verify/README.md`](../outer/verify/README.md) 限界 19）。
9. **回収後の成果物の改変の検出は、ハッシュの一致だけに依る**（`V-13`）。
   1 種類の改変（`frozen/` の `.cshtml` の書き換え）でしか確認しておらず、
   ファイルの追加・削除や `frozen/` の外の改変は確認していない
   （[`outer/verify/README.md`](../outer/verify/README.md) 限界 20）。
