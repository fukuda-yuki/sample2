...
def build(data):
    ...
    for run in data['runs']:
        ...
        for previous,current in zip(calls,calls[1:]):
            if (current.get('input_delta') or 0)<-3000:
                keys=('call_index','request_id','input_tokens','cache_read_tokens','message_count',
                      'system_chars','user_chars','assistant_chars','tool_chars','disappeared_message_count',
                      'request_ref','response_ref')
                decreases.append({'before':{k:previous[k] for k in keys},'after':{k:current[k] for k in keys},
                    'annotation_timing':'post_acquisition','display_threshold':-3000,
                    'boundary':'A provider-reported decrease is not proof of removed input or compaction.'})
        ...
        root=Path(run['root'])
        trace=root/'evidence/agent.jsonl'
        truncations=[]
        by_tool={a['tool_call_id']:a for a in actions}
        if trace.exists():
            for line in trace.read_text(encoding='utf-8-sig').splitlines():
                event=json.loads(line)
                part=event.get('part',{})
                tid=part.get('callID')
                if event.get('type')!='tool_use' or tid not in by_tool:continue
                state=part.get('state',{})
                notices=[s for s in state.get('output','').splitlines()
                    if s.startswith('(Output capped at ') or s.startswith('The tool call succeeded but the output was truncated')]
                if (state.get('metadata') or {}).get('truncated') or notices:
                    a=by_tool[tid]
                    truncations.append({'call_index':a['call_index'],'tool_call_id':tid,
                        'native_ref':a['native_ref'],'metadata_truncated':(state.get('metadata') or {}).get('truncated'),
                        'notices':notices,'annotation_timing':'post_acquisition',
                        'meaning':'Tool output truncation; not a conversation compaction event.'})
        ...
        rows.append({'cohort':run['cohort'],'condition':run['condition'],'run_id':run['run_id'],
            ...
            'large_input_decreases':decreases,
            ...
            'tool_output_truncations':truncations,
