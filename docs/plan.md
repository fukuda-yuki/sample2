...
| D-6 | ユーザー指示（2026-09-15）: 対象は `.NET` で良い。#1〜#5 を Step by step で進め、各段階でコミットする。 | 本 Issue へのユーザー指示 |
| D-7 | `sample2` は後日 private 化する（ユーザー判断）。private 化までは、採点コードと fixture を含む本リポジトリを実装エージェントへ配布しない。 | Issue #4「公開リポジトリへ非公開資材をコミットしない」への対処 |
| D-8 | 旧実装の不具合は保持しない（存在しない識別子は 404、モデル変更で DB を破棄しない、削除時の残数量は 0、住所項目は必須）。 | 課題側の判断。[quality-spec.md](quality-spec.md) §1.1 |
| D-9 | 検査を決定的にするため、金額は `0.00` 形式とし、旧 JavaScript が依存する表示識別子を外部契約として維持する。 | [quality-spec.md](quality-spec.md) §4.5 |

### 未決定事項（今回決めない）

- 移行先言語・ランタイムの一般方針（Java・COBOL 候補の扱いを含む）
- 全面移行か部分移行か、最初の課題の対象範囲（→ #3 で `MS1-001` の範囲を確定済み。§2 と [decision-log.md](decision-log.md) D-10）
- クラウド配備
...
- #4 の課題固有部分は #3 の範囲確定後（#3 の範囲は確定済み）。**課題に依存しない評価器の骨格（起動・観測・判定・記録・再採点）は先に作れる。**
- #5 の移植範囲は #2 の調査結果を踏まえて決める。**#5 で確定した**（[outer-harness.md](outer-harness.md) §9）。
...
3 番目と 4 番目は、**実モデルを動かさない範囲で** #4 と #5 が確認している
（[inner/calibration/README.md](../inner/calibration/README.md)、[outer/verify/README.md](../outer/verify/README.md)）。
**これは実モデル実行での成立を意味しない。** 1 番目・2 番目・5 番目は未達であり、
実モデル接続でしか確認できない残件は [outer-harness.md](outer-harness.md) §10.2 に置いている。
