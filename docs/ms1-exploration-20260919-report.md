...
> **再開中（改訂R1）**：本体以下は初回中断時点の分析である。その後のユーザー指示により、
> [Docker復旧と再開手順](ms1-exploration-20260919-retry-amendment.md)を固定し、残りの初回枠を実行している。
> 元の中断報告は `artifacts/exploration/20260919/recovery-v1/prior-docs` に保持した。
> 最新の実行状態は `runs/exploration-20260919-ms1/batch-journal.jsonl`、再開後の終了結果は
> `resume-v1/result.json`（終了時に生成）を参照。採取終了後に本報告と人の確認対象を更新する。
